...
class TestFactorRegistry:
    ...
    def test_passed_factors(self):
        ...
        registry = FactorRegistry()
        f1 = FactorMetadata(
            factor_id="pass_001", name="f1", agent_id="A1", level=1,
            expression="ts_mean(close, 20)",
            description="Test factor for trend identification",
        )
        f1.review_status = "passed"
        f2 = FactorMetadata(
            factor_id="pass_002", name="f2", agent_id="A2", level=1,
    ...
    def test_structural_dedup(self):
        ...
        registry = FactorRegistry()
        f1 = FactorMetadata(
            factor_id="sd_001", name="f1", agent_id="A1", level=1,
            expression="ts_mean(close, 20)",
            description="Test factor for trend identification",
        )
        f2 = FactorMetadata(
            factor_id="sd_002", name="f2", agent_id="A2", level=2,
